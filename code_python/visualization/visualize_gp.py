...
def _validate_tolerance_percent(tolerance_percent: Optional[float]) -> Optional[float]:
    ...
    try:
        ...
        raise ValueError("tolerance must be a number between 0 and 100 percent.") from exc

    if not np.isfinite(tolerance_percent) or not 0 <= tolerance_percent <= 100:
        raise ValueError("tolerance must be a number between 0 and 100 percent.")
    return tolerance_percent


def _tolerance_rank_row(
    row: pd.Series,
    ...
    ranks = pd.Series(index=row.index, dtype=float)
    first_rank = 1
    for group in groups:
        last_rank = first_rank + len(group) - 1
        ranks.loc[group] = (first_rank + last_rank) / 2.0
...
def kendalls_w(
    ...
    if p_statistics:
        ...
        ranks = pd.DataFrame(
            [
                _tolerance_rank_row(row, tolerance_fraction, tolerance_mode)
                for _, row in df_input.iterrows()
            ],
            index=df_input.index,
            columns=df_input.columns,
...
def plot_model_performance_TOPSIS(
    ...
    ``df`` should be the master ``result_df`` produced by
    ``generate_master_results.build_master_performance_data``. Each metric is
    averaged before TOPSIS by exact model/FP/count/mixing configuration. Tree
    models, which have no kernel columns, remain one row per model. TOPSIS is
    then run on that mean table, with model configurations as alternatives and
    ``metrics`` as criteria. ``include_kernel_config`` is kept only for
    backward-compatible calls and does not collapse selected configurations.
    """
    if not metrics:
...
def plot_hybridization_topsis_comparison(
    ...
    ``df`` should be the master ``result_df`` produced by
    ``generate_master_results.build_master_performance_data``. Each metric is
    averaged for every exact model/FP/count/mixing configuration first. TOPSIS
    is then run on that configuration-mean table, with configurations as
    alternatives and ``metrics`` as the criteria. The bar plot summarizes
    those configuration TOPSIS scores by hybridization method.
